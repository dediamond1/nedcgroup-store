...
export interface SuccessInvoicesProps {
  token: string;
  startDate?: Date | null;
  endDate?: Date | null;
}
...
export function SuccessInvoices({
  token,
  startDate: externalStartDate,
  endDate: externalEndDate,
}: SuccessInvoicesProps) {
  ...
  useEffect(() => {
    const fetchInvoices = async () => {
      ...
      try {
        ...
        if (data.message === "No Success Invoices Data Found") {
          setInvoices([]);
          setFilteredInvoices([]);
          setTotalAmount(0);
        } else {
      ...
      <div className="bg-white p-4 rounded-lg shadow-sm border border-green-100">
        <div className="flex justify-between items-center mb-4">
          <div className="flex items-center gap-2">
            ...
            <h3 className="font-medium">Successful Invoices</h3>
          </div>
        ...
        {loading ? (
          ...
          <div className="space-y-4">
            {paginatedInvoices.map((invoice) => (
              <Card
                key={invoice._id}
                className="overflow-hidden border-l-4 border-l-green-400"
              >
